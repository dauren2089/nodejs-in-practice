...
Промежуточное ПО static позволяет вам объявлять один из каталогов как со-
держащий статические ресурсы, которые довольно просты для того, чтобы их можно 
...
ресурсы в общедоступную папку, чтобы они могли быть выданы клиенту. Мы по-
местим их в подкаталог public/vendor:
```sh
$ mkdir public/vendor
...
Например, если мой контекстный объект { name: 'Лютик' }, а шаблон — <p>Привет, {{name}}!</p>, то {{name}} будет заменено на Лютик.
